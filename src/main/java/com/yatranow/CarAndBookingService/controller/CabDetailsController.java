...
public class CabDetailsController {
	...
	public ResponseEntity<ApiResponse> getCabDetailsByRegId(
	...
	@Operation(summary = "Get cab details by UserID", description = "Retrieves cab details by the given UserID ID.")
	public ResponseEntity<ApiResponse> getCabDetailsByUserId(
			@Parameter(description = "Registration ID of the cab", required = true) @PathVariable("userId") Long userId) {
		try {
			List<CabRegistrationDetails> response = cabRegistrationDetailsRepository.findByUserId(userId);
			return new ResponseEntity<>(new ApiResponse("success", new Object[] { response }, 200), HttpStatus.OK);
		} catch (Exception e) {
			return ResponseEntity.status(500).body(new ApiResponse(e.getMessage(), null, 500));
		}
	...
	public ResponseEntity<ApiResponse> updateCabRegistrationDetails(
		...
		try {
			return cabRegistrationDetailsRepository.findById(registrationId).map(existingDetails -> {
				...
				return new ResponseEntity<>(new ApiResponse("success", new Object[] { savedDetails }, 200),
						HttpStatus.OK);
			}).orElse(new ResponseEntity<>(new ApiResponse("Cab registration details not found", null, 404),
	...
	public ResponseEntity<ApiResponse> deleteCabRegistrationDetails(
		...
		try {
			return cabRegistrationDetailsRepository.findByRegistrationId(registrationId).map(existingDetails -> {
